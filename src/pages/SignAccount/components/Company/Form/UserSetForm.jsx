...
const UserSetForm = (props) => {
  ...
  const handleUpdata = () => {
    ...
    form.validateFields().then((values) => {
      ...
      dispatch({
        type: userInfo.authCompanyId
          ? 'workerManageUser/fetchUserEditByCompany' // 修改
          : 'workerManageUser/fetchUserAddByCompany', // 新增
        payload: {
          ...userInfo,
          ...values,
          password: password ? password : userInfo.authCompanyId ? undefined : mobile.match(/.*(.{6})/)[1],
          roleIds,
        },
        callback: () => {
  ...
  const formItems = [
    {
      ...
      name: 'provinceName',
      // disabled:true,
      select: CITYJSON.map(({value,label}) => ({value:label,name:label})),
    ...
    },
    {
      ...
      name: 'status',
      type: 'radio',
      visible: !!userInfo.authCompanyId,
      select: ['停用', '启用'],
    },
  ];
