...
const MasterOrderDetail = ({ order, name, orderDetail, loading, dispatch }) => {
  ...
  const fetchGetDetail = () => {
    if (!visible)
      dispatch({
        type: 'refundOrder/fetchOrderDetail',
  ...
  return order ? (
    ...
    >
      <a onClick={fetchGetDetail}>订单详情</a>
    </Popover>
  ) : (
    '--'
  );
...
export default connect(({ refundOrder, loading }) => ({
  orderDetail: refundOrder.orderDetail,
  loading: loading.effects['refundOrder/fetchOrderDetail'],
}))(MasterOrderDetail);
