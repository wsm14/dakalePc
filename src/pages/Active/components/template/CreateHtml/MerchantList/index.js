...
${source
  .map((item) => {
    const cellDom = `<div class="handleGoNative"  data-key="merchantName,id" data-merchantName=${
      item.merchantName
  ...
  ${
    ...
    if (native.getPhone() === 'wxChatWebView') {
      return wxOpenLaunchWeapp(cellDom, item.merchantType, item.userMerchantIdString);
    }
    return cellDom;
  })
  .join('')}</div>`;
