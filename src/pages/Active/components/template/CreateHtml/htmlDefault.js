...
export default {
  ...
  globalModal: ({ activityName, head, body, footer }) => {
    ...
    ${footer}
    <script>
    $('body').on('click','.handleGoNative',function(){if(!$(this).attr('data-linkType')&&!$(this).attr('data-native'))return;let keyObj={};($(this).attr('data-key')?$(this).attr('data-key').split(','):[]).map((item)=>{keyObj[item]=$(this).attr('data-'+item)});const path=$(this).attr('data-path');const params={path,params:{...keyObj,hideCurrent:1,},linkType:$(this).attr('data-linkType'),};if($(this).attr('data-linkType')==='H5'){if(native.getPhone()==='dakaleIOS'){window.webkit.messageHandlers['finishAndPushH5'].postMessage(JSON.stringify({path}))}else if(native.getPhone()==='dakaleAndroid'){window.location.href=path;native.nativeInit('close')};return};native.nativeInit($(this).attr('data-native')||'linkTo',params)});
