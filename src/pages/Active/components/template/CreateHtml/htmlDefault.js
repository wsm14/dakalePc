...
export default {
  ...
  globalModal: ({ activityName, head, body, footer }) => {
    ...
    ${footer}
    <script>
    $("body").on("click",".handleGoNative",function(){var t=this;if($(this).attr("data-linkType")||$(this).attr("data-native")){var a={};($(this).attr("data-key")?$(this).attr("data-key").split(","):[]).map(function(i){a[i]=$(t).attr("data-"+i)});var i=$(this).attr("data-path"),e={path:i,hideCurrent:1,params:a,linkType:$(this).attr("data-linkType")};if("H5"!==$(this).attr("data-linkType"))native.nativeInit($(this).attr("data-native")||"linkTo",e);else{if(!i)return void alert("未配置url");var n=i.includes("newPage")?i:i.includes("?")?i+"&newPage=true":i+"?newPage=true";if("dakaleIOS"===native.getPhone())try{window.webkit.messageHandlers.finishAndPushH5.postMessage(JSON.stringify({path:n}))}catch(t){e.path=n,native.nativeInit($(this).attr("data-native")||"linkTo",e)}else"dakaleAndroid"===native.getPhone()&&(native.nativeInit("close"),window.location.href=n)}}}),$("body").on("click",".active_toast1",function(){native.nativeInit("close")}),$("body").on("click",".drawer_close_box",function(){native.nativeInit("close")});
