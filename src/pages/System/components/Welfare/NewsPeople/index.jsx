...
const NewsPeople = (props) => {
  ...
  const fetchGetDetail = (val, type) => {
    const activityStartDay = [moment(val.activityStartDay), moment(val.activityEndDay)];
    setVisible({ show: true, type, detail: { ...val, activityStartDay } });
  };
  ...
  const getColumns = [
    ...
    },
    {
      ...
      dataIndex: 'status',
      render: (val) => (
        <span style={{ color: { 0: '#66ae46', 1: '#333', 2: '#999' }[val] }}>
          {WELFARE_STATUS[val]}
        </span>
      ),
    },
