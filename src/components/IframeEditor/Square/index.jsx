...
import { Tabs, Form, Modal, Radio } from 'antd';
...
const DoubleImg = (props) => {
  ...
  const [selectFormNum, setSelectFormNum] = useState(4); // 图片回显 标题
  ...
  useEffect(() => {
    const oldArr = form.getFieldValue('content');
    let newArr = [];
    if (selectFormNum == 8) {
      newArr = [...oldArr, ...new Array(4).fill({ data: undefined })];
    } else {
      newArr = oldArr.slice(0, 4);
    }
    form.setFieldsValue({ content: newArr });
  }, [selectFormNum]);
      ...
      <Tabs type="card" onChange={setTabs} activeKey={tabs}>
        <Tabs.TabPane tab="自定义" key="1">
          {tabs == 1 && (
            <>
              数量：
              <Radio.Group onChange={(e) => setSelectFormNum(e.target.value)} defaultValue={4}>
              ...
              <Form
                ...
                initialValues={
                  initialValues
                    ...
                    : { content: new Array(selectFormNum).fill({ data: undefined }) }
                }
                layout="vertical"
